---
title: "EDA in R: Practice With Gapminder Data"
...
This exercise is based on data gathered and prepared for Data Investigation in Python course project. The project report can be found [here](https://tianaq.github.io/udacity-bertelsmann-ds-challenge/5-dand-data-investigation/Investigate_a_Dataset.html). 
...
```{r loading_data}
mat_mort_data <- read.csv('data/maternal_mortality_ratio_per_100000_live_births.csv')
wb_income_groups <- read.csv('data/WB_income.csv')
regions <- read.csv('data/list-of-countries-etc.csv')
...
```{r adding_total_fertility}
total_fert <- read.csv('data/children_per_woman_total_fertility.csv')
